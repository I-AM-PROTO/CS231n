...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        X = np.concatenate((X, np.ones((num_train, 1))), axis=1)
        Wb1 = np.concatenate((W1, np.reshape(b1, (1,-1))), axis=0)
        
        H = np.dot(X,Wb1)
        relu_mask = H<0
        H[relu_mask] = 0
        
        H = np.concatenate((H, np.ones((num_train, 1))), axis=1)
        Wb2 = np.concatenate((W2, np.reshape(b2, (1,-1))), axis=0)
        ...
        pred = -np.log(scores[np.arange(num_train), y])
        loss = np.sum(pred)
        loss /= num_train
        loss += reg * (np.sum(Wb1 * Wb1) + np.sum(Wb2 * Wb2))
        ...
        dWb2 = np.dot(np.transpose(H), dS)
        dWb2 += 2 * reg * Wb2
        
        dH = np.dot(dS, np.transpose(Wb2))[:, 0:-1]
        dH[relu_mask] = 0
